...
### 機械学習チームの引き継ぎ（2024/06-）
機械学習チームのマネジメントを引き継いだ。

#### 詳細
前任者の退職により、機械学習モデルの実装・保守・導入を担当するチームのマネージャーを引き継いだ。
...
候補を2つに絞って観点表を作成し、期間を定めてプロジェクトメンバーとステークホルダーに試用してもらったこと。

試用結果を元に、プロジェクトメンバーと話し合って意思決定を行ったこと。

### チーム内ドキュメント作成（2023/10-）
...
他チームがエンジニアリングマネージャー（EM）とエンジニアの採用活動を行うにあたり、求人票の取りまとめと選考活動を行う。
...
アプリケーションのビルドに利用しているNode.jsのバージョンを上げ、パッケージマネージャをyarnからnpmへと変更する。
...
アプリケーションで利用しているRubyのバージョンをEnd of Life直前の2.7から当時の最新である3.2へアップグレードする。
...
アプリケーションで利用しているAmazon Aurora を、v1（MySQL 5.6互換）からv2（5.7互換）にアップグレードする。
...
Amazon Elasticsearch Service（現：Amazon Opensearch Service）を利用して、アプリケーションに検索エンジンを導入する。
...
特定の法人によるアプリケーション内での活動を集約するための機能として、法人マスタを開発する。
...
ClassicASPでWebアプリケーションとして作られた管理会計システムの開発および保守を行う。
