...
CTOとともに、会社全体の開発組織をマネジメントする。また、経営会議に参画する。

更新時点で会社で開発運用しているプロダクトは、自由診療クリニック特化/Webベースの基幹システム「[B4A](https://www.b4a.co.jp/)」のみ。

以下を実施予定。実施が完了したら追記。

* 開発体制の刷新
* プロダクト開発へのメトリクス導入および運用
* エンジニア採用
* エンジニア採用広報
...
一貫して製造業向けビジネスマッチングプラットフォーム「[Linkers Sourcing](https://corp.linkers.net/service/ls/) / [Marketing](https://corp.linkers.net/service/lm/)」（LS/LM）の開発運用を行う。

ポジション変更をまたいで進行したPJもあるため、以下の職務経歴ではポジションの境界は記さない。

### コード生成AIの導入（2025/04-07）
...
### Findy Team+を利用したメトリクスの運用（2024/12-退職まで）
LS/LM開発チームにFindy Team+を利用したメトリクスを導入した。また、それを運用する。

#### 詳細
LS/LM開発チームには以前よりメトリクスを作成していた。

これは「**質とスピードを両立しつつ、安定してデリバリーできているか**」を見るためのもので、保守開発においてベロシティの安定性・不具合の数・イシューの消化ペースを測定していた。

しかし、以下の理由から従来のメトリクスを計測する意味が薄くなっていた。

* スクラムの運用が改善されたことで、ベロシティが定点観測の必要がないほど安定した
* 障害発生時に都度ふりかえりを行い、エラーを検知するたび解消したことで数週に一度しか不具合が起きない
* データ構造の改善が終わったことで開発フェーズが変わり、チケットの粒度が大きくなった

また、これらは一部が手動で運用されていたため手間がかかっていた。

以上のような事情から、

* 目的を変えずに、現在（＝メトリクス見直し当時）のチームにあったメトリクスを再定義する
* メトリクスの運用コストを低減する

ことを目指してFindy Team+を導入し、新たなメトリクスを設定して運用に乗せた。

新しいメトリクスは

* 過去28日間における[Four keys](https://cloud.google.com/blog/ja/products/gcp/using-the-four-keys-to-measure-your-devops-performance)（デプロイ頻度・変更のリードタイム・変更障害率・サービス復元時間）
* 過去7日間におけるPull Requestのサイクルタイム

であり、これを週次のMTGで確認している。

#### 工夫した点
項目の見直しにあたり改めてメトリクス取得の目的を言語化し、それに沿って新しい項目を選定したこと。

項目の設定にあたって叩き台を作り、Findy社のカスタマーサクセスの方にレビューを受けて他に良い項目がないかを確認したこと。

項目が確定したら、各項目を測定する理由をドキュメントにまとめ、チーム全員で確認して承認を取ったこと。
...
GitLab+GitLab CIで行っていたコードのホスティングとCIを、GitHub+CircleCIに移行した。
...
### 他チームのEM/エンジニア採用企画・選考（2023/09-退職まで）
...
アプリケーション内で利用する画像処理ライブラリをImageMagickからlibvipsに入れ替えた。
...
### 脆弱性診断の発注側担当（2021/04-07, 2022/12-23/02, 2024/01-03, 2025/01-03）
...
### 技術カンファレンスへのスポンサー事務（2018/02-退職まで）
...
2023年以降は予算取りからノベルティの準備、メンバーの経費精算のサポートまで行っている。また、Kaigi on Railsなど他のカンファレンスへのスポンサードを進めている。
